...
fn main() -> Result<()> {
    ...
    let tokenizer_path = "tokenizer_mixed.json";
    let model_path = "mixed_xlstm_model.safetensors";
    ...
    if !existe_modelo || continuar_entrenamiento {
        ...
        let mut optim_mlstm = AdamW::new(mlstm_params, ParamsAdamW { lr: 3e-4, ..Default::default() })?; // Subido de 3e-4
        let mut optim_mingru = AdamW::new(mingru_params, ParamsAdamW { lr: 1e-3, ..Default::default() })?; // Subido de 6e-4
        let mut optim_minlstm = AdamW::new(minlstm_params, ParamsAdamW { lr: 8e-4, ..Default::default() })?; // Subido de 3e-4
