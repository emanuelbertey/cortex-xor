...
fn main() -> Result<()> {
    ...
    for i in 1..=num_models {
        ...
        models.push(model);
        {
            ...
            let optim = AdamW::new(params, ParamsAdamW { lr: 2e-3, ..Default::default() })?;
    ...
    let mut final_optimizer = AdamW::new(final_params, ParamsAdamW { lr: 3e-3, ..Default::default() })?;
    ...
    if train_mode {
        ...
        for epoch in 0..num_epochs {
            ...
            for batch_idx in start_batch..num_batches {
                ...
                std::thread::scope(|s| {
                    ...
                    for (_m_idx, (model, optimizer)) in models.iter().zip(optimizers.iter_mut()).enumerate() {
                        ...
                        handles.push(s.spawn(move || {
                            ...
                            let mut state = None;
                            let mut step_logits_list = Vec::with_capacity(seq_length);

                            for t in 0..seq_length {
                                let input_step = inp.narrow(1, t, 1)?;
                                
                                // 1. Usar el embedding propio del modelo
                                let x_emb = if let Some(emb) = &model.embedding {
                                    emb.forward(&input_step)?
                                } else {
                                    input_step.clone()
                                };

                                // 2. Aplicar la proyección de entrada
                                let x_in = if let Some((linear, norm, _)) = &model.input_projection {
                                    let x = linear.forward(&x_emb)?;
                                    let x = norm.forward(&x)?;
                                    x.gelu()?
                                } else {
                                    x_emb
                                ...
                                // 3. Ejecutar bloque individualmente
                                let (out_b, next_st) = model.blocks[0].forward(&x_in, state.and_then(|mut v: Vec<Option<LSTMState>>| v.remove(0)))?;
                                
                                let h = if let Some(LSTMState::SLSTM(ref v)) = next_st {
                                    v.last().unwrap().hidden.detach()
                                } else {
                                    Tensor::zeros(out_b.dims(), DType::F32, &inp.device())?
                                };

                                if t % attn_stride == 0 {
                                    memories.push_back(h.clone());
                                    if memories.len() > attn_len { memories.pop_front(); }
                                }

                                // Aplicar Luong Attention real
                                ...
                                let logits_step = lin2.forward(&x)?.reshape((batch_size, vocab_size))?;
                                
                                step_logits_list.push(logits_step);
                                state = Some(vec![next_st.map(|s| s.detach())]);
                            }

                            let model_logits_seq = Tensor::cat(&step_logits_list, 0)?.reshape((batch_size * seq_length, vocab_size))?;
                            let loss = candle_nn::loss::cross_entropy(&model_logits_seq, &tar)?;
                            ...
                            Ok::<(f32, Vec<Tensor>), anyhow::Error>((loss_val, step_logits_list))
                        }));
                    }

                    for handle in handles {
                        let (loss_val, logits_list) = handle.join().unwrap().expect("Error en hilo de modelo");
                        model_losses.push(loss_val);
                        ensemble_logits_results.push(logits_list);
                    }
                });
                ...
                // --- 2. ENTRENAMIENTO DEL MODELO FINAL (Secuencia con Atención Estridente) ---
                let mut final_attn = SequentialTwoSLstm::new(&final_model, attn_len, attn_stride, AttentionScope::Both);
                let mut final_logits_list = Vec::with_capacity(seq_length);
                let mut ensemble_avg_list = Vec::with_capacity(seq_length);

                for t in 0..seq_length {
                    // Promediar logits del ensemble para este paso
                    let mut sum_logits = ensemble_logits_results[0][t].clone();
                    for m in 1..num_models {
                        sum_logits = (sum_logits + &ensemble_logits_results[m][t])?;
                    }
                    let avg_logits = (sum_logits / (num_models as f64))?;
                    ensemble_avg_list.push(avg_logits.clone());

                    // Construir entrada con Residuo
                    let input_step = input_batch.narrow(1, t, 1)?.flatten_all()?.to_vec1::<u32>()?;
                    let mut one_hot_data = vec![0.0f32; batch_size * vocab_size];
                    for (b, &idx) in input_step.iter().enumerate() {
                        one_hot_data[b * vocab_size + (idx as usize)] = 1.0;
                    }
                    let residue = Tensor::from_vec(one_hot_data, (batch_size, vocab_size), &device)?;
                    
                    let averaged_norm = ln_comb.forward(&avg_logits)?;
                    let sum_with_residue = averaged_norm.add(&residue)?;
                    let after_drop = drop_comb.forward(&sum_with_residue, true)?;
                    let combined_input = after_drop.reshape((batch_size, 1, vocab_size))?;

                    let final_step_logits = final_attn.step(&combined_input)?;
                    final_logits_list.push(final_step_logits);
                }

                let final_logits = Tensor::cat(&final_logits_list, 1)?;
                ...
                // Calcular pérdida del ensemble consolidado para el log
                let ens_logits_flat = Tensor::cat(&ensemble_avg_list, 0)?;
