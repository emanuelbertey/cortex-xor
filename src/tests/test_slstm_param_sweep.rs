/*!
    Test de estrés y barrido de parámetros para sLSTM.
    Barre: weight_stdev, forget_bias, input_bias, epsilon, exp_clamp_min/max, stabilizer_init.
    Reporta estabilidad numérica y convergencia de entrenamiento.
*/

use candle_core::{DType, Device, Result, Tensor, D};
use candle_nn::{AdamW, Optimizer, ParamsAdamW, VarBuilder, VarMap};
use xlstm::SLstmconfig;
use std::env;
...
fn run_stability(cfg: &SLstmconfig, device: &Device) -> Result<bool> {
...
fn train_once(cfg: &SLstmconfig, steps: usize, device: &Device) -> Result<((f32, f32), (f32, f32))> {
    let varmap = VarMap::new();
    let vb = VarBuilder::from_varmap(&varmap, DType::F32, device);
    let model = cfg.init(vb)?;
...
fn build_sweep_cases(dim: usize) -> Vec<SweepCase> {
    let base = || SLstmconfig::new(dim, dim, 1);
    vec![
        // ─── 1. Baseline (defaults) ───
        SweepCase {
        ...
        // ─── 3. forget_bias sweep ───
        SweepCase {
            label: "forget_bias=-2.0 (olvido rápido)".into(),
            cfg: { let mut c = base(); c.forget_bias = -2.0; c },
        },
        SweepCase {
        ...
        // ─── 4. input_bias sweep ───
        SweepCase {
            label: "input_bias=-1.0 (escritura lenta)".into(),
            cfg: { let mut c = base(); c.input_bias = -1.0; c },
        },
        SweepCase {
            label: "input_bias=0.5 (escritura media)".into(),
            cfg: { let mut c = base(); c.input_bias = 0.5; c },
        },
        SweepCase {
            label: "input_bias=2.0 (escritura agresiva)".into(),
            cfg: { let mut c = base(); c.input_bias = 2.0; c },
        },

        // ─── 5. exp_clamp sweep ───
        SweepCase {
            label: "exp_clamp=[-10, 0] (restringido)".into(),
            cfg: { let mut c = base(); c.exp_clamp_min = -10.0; c.exp_clamp_max = 0.0; c },
        },
        SweepCase {
            label: "exp_clamp=[-50, 0] (amplio)".into(),
            cfg: { let mut c = base(); c.exp_clamp_min = -50.0; c.exp_clamp_max = 0.0; c },
        },
        SweepCase {
            label: "exp_clamp=[-100, 0] (muy amplio)".into(),
            cfg: { let mut c = base(); c.exp_clamp_min = -100.0; c.exp_clamp_max = 0.0; c },
        },
        ...
        // ─── 7. stabilizer_init sweep ───
        SweepCase {
            label: "stabilizer_init=-20.0 (muy negativo)".into(),
            cfg: { let mut c = base(); c.stabilizer_init = -20.0; c },
        },
        SweepCase {
            label: "stabilizer_init=-5.0 (poco negativo)".into(),
            cfg: { let mut c = base(); c.stabilizer_init = -5.0; c },
        },
        SweepCase {
            label: "stabilizer_init=0.0 (sin bias)".into(),
            cfg: { let mut c = base(); c.stabilizer_init = 0.0; c },
        },

        // ─── 8. use_separate_bias = false ───
        SweepCase {
            label: "use_separate_bias=false (bias uniforme)".into(),
            cfg: { let mut c = base(); c.use_separate_bias = false; c },
        },

        // ─── 9. Combinaciones extremas ───
        SweepCase {
            label: "COMBO: agresivo (ws=0.1, fb=-1.0, ib=1.0, m0=-5.0)".into(),
            cfg: {
                let mut c = base();
                c.weight_stdev = 0.1;
                c.forget_bias = -1.0;
                c.input_bias = 1.0;
                c.stabilizer_init = -5.0;
                c
            },
        },
        SweepCase {
            label: "COMBO: conservador (ws=0.005, fb=2.0, ib=-0.5, m0=-20.0)".into(),
            cfg: {
                let mut c = base();
                c.weight_stdev = 0.005;
                c.forget_bias = 2.0;
                c.input_bias = -0.5;
                c.stabilizer_init = -20.0;
                c.exp_clamp_min = -50.0;
                c
            },
        },
        SweepCase {
            label: "COMBO: paper-like (ws=0.02, fb=0.0, eps=1e-6, m0=-10.0)".into(),
            cfg: base().with_stability(0.02, 0.0, 1e-6),
        },
    ]
}
...
fn main() -> Result<()> {
    ...
    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║        sLSTM PARAMETER SWEEP — STRESS TEST                     ║");
    println!("╚══════════════════════════════════════════════════════════════════╝");
    println!("  dim={}, train_steps={}", dim, train_steps);
    println!();

    let cases = build_sweep_cases(dim);
    ...
    for (i, case) in cases.iter().take(total).enumerate() {
        ...
        println!("   Params: ws={:.4}, fb={:.2}, ib={:.2}, eps={:.1e}, ec=[{:.1},{:.1}], m0={:.1}, sep_bias={}",
            case.cfg.weight_stdev,
            case.cfg.forget_bias,
            case.cfg.input_bias,
            case.cfg.epsilon,
            case.cfg.exp_clamp_min,
            case.cfg.exp_clamp_max,
            case.cfg.stabilizer_init,
