...
static ATTN_VECTORS: AtomicUsize = AtomicUsize::new(1); //VECTOR DEL CONTESTO
static ATTN_STRIDE_VAL: AtomicUsize = AtomicUsize::new(64); // STRIDE GLOBAL
static GLOBAL_DROPOUT: f32 = 0.1; //DROPOUT PARA TODO 
...
impl<'a> ParallelSLstmAttention<'a> {
    fn new(models: &'a [XLstm], vocab_size: usize, attn_len: usize, attn_stride: usize, local_len: usize, _device: Device, barrier: Arc<Barrier>, train: bool) -> Self {
        let memories = (0..models.len()).map(|_| VecDeque::with_capacity(attn_len)).collect();
        let states = vec![None; models.len()];
        Self { models, states, memories, attn_len, attn_stride, step_count: 0, local_len, vocab_size, barrier, train }
    }
    fn step(&mut self, input: &Tensor) -> Result<(Tensor, Tensor)> {
            ...
            for (i, model) in self.models.iter().enumerate() {
                ...
                handles.push(s.spawn(move || {
                    ...
                    let x = if let Some((linear, norm, _)) = &model.input_projection {
                     ...
                     /*
                         se puede ajustar el modelo a mas bloques por simplicidad es solo 1 vector.0 
                         genera mas lactancia y tiempo es mejor tener mas podelos paralelos y no mas 
                         modelos apilados en paralelos ;
                         modelos apilados pueden afinar muho mas :)

                     */
                    // ajustar en for para mas modelos en incersion 
        ...
        for (i, (out_b, next_st)) in results.into_iter().enumerate() {
            ...
            let ctx = if !self.memories[i].is_empty() {
                let keys = stack_states(&self.memories[i])?;
                let (ctx, _) = luong_attention(&out_b, &keys)?;
                ctx
            } else {
                Tensor::zeros(out_b.dims(), out_b.dtype(), out_b.device())?
            };
            ...
            let x = drop.forward(&x, self.train)?;
...
impl<'a> SequentialTwoSLstm<'a> {
    fn new(final_model: &'a XLstm, attn_len: usize, attn_stride: usize, scope: AttentionScope, train: bool) -> Self {
        Self {
            model: final_model,
...
fn generate_threaded_text(
    ...
    // let attn_len = ATTN_VECTORS.load(Ordering::Relaxed);
    // let attn_stride = 128;    // En inferencia solemos usar un stride corto

    // Inicializar wrappers de atención
    let mut parallel_attn = ParallelSLstmAttention::new(models, tokenizer.vocab_size(), attn_len, attn_stride, 2, device.clone(), barrier.clone(), false);
    let mut final_attn = SequentialTwoSLstm::new(final_model, attn_len, attn_stride, AttentionScope::Both, false);
    ...
    for &t in &current_tokens {
        ...
        let combined_input = sum_with_residue.unsqueeze(1)?;
...
fn main() -> Result<()> {
    ...
    for i in 1..=num_models {
        ...
        models.push(model);
        {
            ...
            let optim = AdamW::new(params, ParamsAdamW { lr: 1.5e-3, ..Default::default() })?;
    ...
    let mut final_optimizer = AdamW::new(final_params, ParamsAdamW { lr: 1.5e-3, ..Default::default() })?;
    ...
    if train_mode {
        ...
        for epoch in 0..num_epochs {
            ...
            for batch_idx in start_batch..num_batches {
                ...
                let attn_len = ATTN_VECTORS.load(Ordering::Relaxed);
                let attn_stride = ATTN_STRIDE_VAL.load(Ordering::Relaxed);
                ...
                let (final_logits, _) = final_model.forward(&sum_with_residue, None)?; 
                ...
                print!("{}", log_str);
                io::stdout().flush()?;
                if batch_idx > start_batch && batch_idx % 21 == 0 {
                    ...
                    let attn_len_val = ATTN_VECTORS.load(Ordering::Relaxed);
                    let attn_stride_val = 128; // Stride corto para inferencia/muestreo
                    let gen_sample = generate_threaded_text(&models, &final_model, &ln_comb, &tokenizer, &seed, 300, attn_len_val, attn_stride_val, &device, &barrier_gen)?;
                    println!("  Luong-sLSTM: {}\n", gen_sample);
                }
            }
        }
    ...
    if train_mode {
        ...
        let attn_len_val = ATTN_VECTORS.load(Ordering::Relaxed);
        let gen_final = generate_threaded_text(&models, &final_model, &ln_comb, &tokenizer, &seed, 256, attn_len_val, 128, &device, &barrier)?;
        println!("{}", gen_final);
    } else {
        loop {
            ...
            print!("Tamaño del vector [Enter=0]: ");
            io::stdout().flush()?;
            let mut v_len_str = String::new();
            io::stdin().read_line(&mut v_len_str)?;
            let v_len = v_len_str.trim().parse::<usize>().unwrap_or(0);
            
            // Actualizamos el valor global para que el resto del sistema lo sepa
            ATTN_VECTORS.store(v_len, Ordering::Relaxed);

            // Re-inicializamos el barrier para cada generación si es necesario
            let barrier = Arc::new(Barrier::new(num_models));
            let out = generate_threaded_text(&models, &final_model, &ln_comb, &tokenizer, &use_seed, len, v_len, 128, &device, &barrier)?;
            println!("{}", out);
        }
    }
    Ok(())
